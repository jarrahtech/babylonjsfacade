...
trait XRHitTestResult extends StObject {
  ...
  def getPose(baseSpace: EventTarget): js.UndefOr[XRPose]
}
object XRHitTestResult {
  
  inline def apply(getPose: EventTarget => js.UndefOr[XRPose]): XRHitTestResult = {
  ...
  implicit open class MutableBuilder[Self <: XRHitTestResult] (val x: Self) extends AnyVal {
    ...
    inline def setGetPose(value: EventTarget => js.UndefOr[XRPose]): Self = StObject.set(x, "getPose", js.Any.fromFunction1(value))
  }
}
